...
export function MinisterCard({ name, image, position, officer, greeting }: MinisterCardProps) {
  ...
  return (
    <article className="group border border-[#E6E6E6] bg-white p-6 shadow-[0_1px_2px_rgba(30,42,74,0.04)] transition hover:border-[#1e2a4a]/18 hover:shadow-md sm:p-8">
      <div className="flex flex-col gap-6 md:flex-row md:items-start md:gap-10">
        <div className="relative mx-auto size-[112px] shrink-0 overflow-hidden rounded-full border-2 border-[#1e2a4a]/12 shadow-inner ring-4 ring-[#1e2a4a]/[0.06] sm:size-[128px] md:mx-0 md:size-[140px]">
          <Image
            src={image}
            alt={`${name} ${title}`}
            fill
        ...
        <div className="min-w-0 flex-1 space-y-3 text-center md:text-left">
          <div className="space-y-1">
            <h3 className="text-xl font-bold tracking-tight text-[#1e2a4a] sm:text-2xl">
              {name}{" "}
              <span className="font-semibold text-[#496674]">{title}</span>
            </h3>
