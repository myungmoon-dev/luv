...
type DiscipleshipAlbumGridSectionProps = {
  albumType: AlbumType;
  albumName: string;
  description?: string;
};

export function DiscipleshipAlbumGridSection({
  albumType,
  albumName,
  description,
}: DiscipleshipAlbumGridSectionProps) {
  const [page, setPage] = useState(1);
  const [lightboxUrl, setLightboxUrl] = useState<string | null>(null);

  const { data, isPending, isError } = useQuery({
    queryKey: ["albums", albumType, page],
    queryFn: () => getAlbumList({ type: albumType, page, size: ITEMS_PER_PAGE }),
  });

  const albums = data?.albums ?? [];
  const totalItems = data?.totalAlbums ?? 0;

  return (
    <>
      <section className="mt-12 border-t border-[#E6E6E6] pt-10">
        <div className="mb-6 flex flex-col gap-2 sm:mb-8">
          <h2 className="text-xl font-bold text-[#1e2a4a] sm:text-2xl">{albumName}</h2>
          {description ? <p className="text-sm text-[#496674]">{description}</p> : null}
        </div>
