...
const Map = ({ isWhite, address, title }: IMapProps) => {
  return (
    ...
    >
      <div className={cn("flex w-4/5 flex-col", isWhite ? "items-start text-white" : "items-end")}>
        <p className={cn("text-lg font-extrabold sm:text-xl md:text-2xl", !isWhite && "text-blue-600")}>{title}</p>
        <p className="mb-2 text-lg font-bold sm:text-xl md:text-2xl">{address}</p>
    ...
    <div className="relative">
      ...
      <div className="absolute left-0 top-0 flex w-full flex-col items-center gap-8 px-8 pb-16 pt-24">
        <div className="flex flex-col items-center gap-3">
          <p data-aos="fade-up" data-aos-duration="1000" className="text-4xl font-bold text-blue-600 md:text-5xl">
            '명문교회' 오시는 길
          </p>
          <p
            data-aos="fade-up"
