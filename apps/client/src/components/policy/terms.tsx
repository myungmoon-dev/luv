...
const POLICY_DATA = [
  ...
  },
  {
    id: 17,
    title: "부칙",
    content: "이 약관은 2024년 6월 12일부터 시행됩니다.",
  },
];
...
const PolicyTermsContent = () => {
  return (
    <div
      data-aos="fade-left"
      className="mb-20 flex w-full flex-col items-center justify-center gap-10 overflow-x-hidden"
    >
      {POLICY_DATA.map((data, idx) => (
        <div className="flex w-full flex-col gap-2 px-10">
          <p className="text-xl font-bold text-blue-600">
            {idx !== POLICY_DATA.length - 1 ? `제 ${data.id}조 ${data.title}` : data.title}
          </p>
          <p className="whitespace-pre-wrap break-keep">{data.content}</p>
        </div>
      ))}
    </div>
  );
