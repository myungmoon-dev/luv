...
const MinisterFormDialog = ({ open, onClose, tabType, target }: Props) => {
  ...
  const { register, handleSubmit, reset, control, formState: { errors } } = useForm<MinisterForm>();
  ...
  useEffect(() => {
    if (open && target) {
      ...
      reset({ name: "", position: "", officerType: "", tabType, greeting: "", description: "" });
      setImage(null);
      setPreview(null);
    }
  ...
  const onSubmit = (form: MinisterForm) => {
    ...
    if (isEdit) {
      patch(
        ...
        {
          onSuccess: () => { toast.success("수정되었습니다."); handleClose(); },
      ...
      post(
        formData,
        {
          onSuccess: () => { toast.success("추가되었습니다."); handleClose(); },
          onError: () => toast.error("에러가 발생했습니다."),
        },
      );
    }
    ...
    <Dialog open={open} onOpenChange={(o) => !o && handleClose()}>
      <DialogContent className="flex h-[90dvh] max-h-[90dvh] flex-col gap-0 p-0 sm:max-w-lg">
            ...
            {!isRetiredPastor && (
              <>
                ...
                <div className="flex flex-col gap-1.5">
                  ...
                  />
                  {errors.officerType && <p className="text-destructive text-xs">{errors.officerType.message}</p>}
                </div>
                ...
                {showGreeting && (
                  <div className="flex flex-col gap-1.5">
                    ...
                    />
                    {errors.greeting && <p className="text-destructive text-xs">{errors.greeting.message}</p>}
                  </div>
                )}
              </>
            )}
        ...
        <div className="bg-background shrink-0 border-t px-6 py-4">
          <Button form="minister-form" className="w-full" disabled={isPending} isLoading={isPending}>
            {isEdit ? "수정" : "추가"}
          </Button>
        </div>
      </DialogContent>
