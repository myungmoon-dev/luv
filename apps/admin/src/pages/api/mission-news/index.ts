import { api } from "@/api";
import { getMissionNewsList, getMissionNewsListCount, postMissionNews } from "firebase";
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  switch (method) {
    case "GET":
      ...
      const missionNewsList = (await getMissionNewsList({ lastVisibleCreatedAt })).docs.map(
        (doc) => ({
          ...doc.data(),
          id: doc.id,
        }),
      );
      const totalMissionNewsListCount = (await getMissionNewsListCount()).data().count;

      return res.status(200).json({
        missionNewsList,
        totalMissionNewsListCount,
      });
    ...
    case "POST":
      ...
      upload.single("image-file")(req as any, res as any, async (err) => {
        ...
        const result = await postMissionNews({
          content: fields.content,
          date: fields.date,
          image: contentImage,
          title: fields.title,
          createdAt: new Date().getTime(),
          writer: fields.userName,
        });
